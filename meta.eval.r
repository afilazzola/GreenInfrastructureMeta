...
meta.temp1 <- meta.data %>%  filter(!!site %in% ex1[,1]) %>% filter(!!stat =="count") %>% 
    dcastMult(f1, funs=list("mean"=mean,"sd"=sd, "n"=length), value.var = "Value")  ## calculate mean, sd, and length (N)
...
meta.temp2 <- meta.data %>%  filter(!!site %in% ex1[,1]) %>% filter(!!stat =="mean" | !!stat =="sd") %>% 
  dcastMult(f2, funs=list("mean"=mean), value.var = "Value") ## extracted mean and SD

## Get replicate value (N)
meta.temp3 <- meta.data %>%  filter(!!site %in% ex1[,1]) %>% filter(!!stat =="mean") %>% dcastMult(f1, funs=list("mean"=mean), value.var = "Rep") 
