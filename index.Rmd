...
library(metafor)
library(bootES)
#http://link.springer.com/article/10.3758%2Fs13428-013-0330-5#Sec36
...
## Generate a Function that calculates bootstrapped Cohen's D for each variable.
effcalc <- function(dat, group) {
eff.vals <- bootES(dat[,c(group,"Value")], R=999, data.col="Value", group.col=group, effect.type="cohens.d", contrast=c(green=1,grey=-1))
eff.output <- summary(eff.vals)
rownames(eff.output) <- unique(dat[,"Study"])
return(eff.output)
}

## Identify studies where the comparison is possible
ex1 <- meta %>% group_by(Study) %>% summarize(compare.true=length(unique(Infrastructure))) %>% filter(compare.true==2)
ex1 <- ex1[1:10,] ## drop last two studies because presence-only

## Conduct bootstrapping on all studies possible
ex2 <- meta %>% filter(Study %in% ex1$Study) %>% group_by(Study) %>% do(fit = effcalc(dat=., group="Infrastructure"))
cleaned <-tidy(ex2, fit)


## Identify groups to be compare
treat <- "green roof"
control <- "conventional roof"


## Identify studies where the comparison is possible
ex1 <- meta %>% group_by(Study) %>% summarize(compare.true=length(unique(GI.type))) %>% filter(compare.true==2)
ex1 <- ex1[1:10,] ## drop last two studies because presence-only

##Bootstrapped cohen's D for different green infrastructures
effcalc <- function(dat, group) {
eff.vals <- bootES(dat[,c(group,"Value")], R=999, data.col="Value", group.col=group, effect.type="cohens.d", contrast=c(green=1,grey=-1))
eff.output <- summary(eff.vals)
rownames(eff.output) <- unique(dat[,"Study"])
...
## extract means for each type of infrastructure
gi.contrast <- meta %>% group_by(Study, Infrastructure) %>% summarize(eff=mean(Value), error=se(Value))
gi.contrast <- data.frame(gi.contrast)

## find studies that overlap (i.e. have both green and grey)
overlap <- gi.contrast[duplicated(match(gi.contrast[,1],gi.contrast[,1])),] ## identify studies that overlap
eff.contrast <- gi.contrast[gi.contrast$Study %in% overlap$Study,] ## extract the overlap studies only
