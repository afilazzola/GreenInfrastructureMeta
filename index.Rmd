...
library(reshape2)
library(metafor)
source("dcastMult.R") ## Multiple aggregate


## Create Unique identifier column
meta2 <- meta
meta2[,"UniqueSite"] <- paste(meta2$Study,meta2$City, meta2$SiteID, meta2$Taxa,  meta$Estimate, sep="-")


## Identify studies where the comparison is possible
ex1 <- meta2 %>% group_by(UniqueSite) %>% summarize(compare.true=length(unique(Infrastructure))) %>% filter(compare.true==2)


meta.data <- meta2 %>%  filter(UniqueSite %in% ex1$UniqueSite) %>% filter(Stat=="mean" | Stat=="count") %>% 
    dcastMult(UniqueSite ~ Infrastructure, funs=list("mean"=mean,"sd"=sd, "n"=length), value.var = "Value") 
meta.data[is.na(meta.data)] <- 1

meta.ready <- escalc(n1i = grey_n, n2i = green_n, m1i = grey_mean, m2i = green_mean, sd1i = grey_sd, sd2i = green_sd, data = meta.data, measure = "SMD", append = TRUE)
...
forest(m1, slab = meta.data$UniqueSite)
