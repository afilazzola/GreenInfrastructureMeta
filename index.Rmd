...
GI.data <- rbind(gf.data,garden.data)
## add column for data type
GI.data[,"GI.type"] <- c(rep("Green Roof",nrow(gf.data)),rep("Community Garden",nrow(garden.data)))
...
m <- leaflet(data=GI.data) %>%
  addTiles() %>%  
   addCircleMarkers(~lon, ~lat, color =  ifelse(GI.data$GI.type == "Green Roof", 'blue', 'red'), radius=5)
m
```